  ...
  async headers() {
    return [{
      ...
      headers: [
        ...
        { key: "Content-Security-Policy", value: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' *.clerk.accounts.dev; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: *.tiktokcdn.com *.tiktokcdn-us.com *.tiktokcdn-eu.com *.clerk.com https://img.clerk.com; connect-src 'self' *.clerk.accounts.dev *.clerk.com https://clerk-telemetry.com https://*.clerk-telemetry.com *.polar.sh vitals.vercel-insights.com; font-src 'self'; frame-src 'self' *.clerk.accounts.dev https://challenges.cloudflare.com; worker-src 'self' blob:" },
      ],
    }];
  },
  async redirects() {
